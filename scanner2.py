...
Uses TCP connect-based discovery and concurrent port scanning.
...
class NetworkScanner:
    ...
    def discover_hosts(self) -> List[str]:
        """
        Identifies live hosts using TCP connect probes on common ports.
        Works cross-platform and inside Docker without raw sockets.
        """
    ...
    def _scan_single_port(self, ip: str, port: int) -> Optional[int]:
        """Scans a single TCP port on a specific IP."""
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.settimeout(self.timeout)
    ...
    def scan_ports(self, live_ips: List[str]) -> None:
        """Scans ports on live hosts using concurrent threads."""
